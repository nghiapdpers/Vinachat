...
export default function listGroupChatReducer(
  state = initialState,
  action: AnyAction,
) {
  switch (action.type) {
    ...
    case GROUPCHAT.UPDATE_LATEST_MESSAGE:
      ...
      updatedDataArray?.forEach((updatedItem: any) => {
        const index = updatedData.findIndex(
          (item: any) => item?.ref === updatedItem.ref,
        );
        // console.log('vị trí index:>>', index);

        // Nếu tìm thấy mục với cùng ref và khác latest_message_ref thì cập nhật "latest_message_text"
        if (
          index !== -1 &&
          updatedData[index].latest_message_ref !==
            updatedItem.latest_message_ref &&
          updatedItem.latest_message_ref
        ) {
          // Cập nhập nội dung tin nhắn
          updatedData[index].latest_message_text =
            updatedItem.latest_message_text;
          updatedData[index].latest_message_type =
            updatedItem.latest_message_type;
          // Cập nhập tên người gửi tin nhắn
          updatedData[index].latest_message_from_name =
            updatedItem.latest_message_from_name;
          // Cập nhập latest_message_ref
          updatedData[index].latest_message_ref =
            updatedItem.latest_message_ref;
      ...
      const unchangedData = updatedData.filter(
        (item: any) =>
          !updatedTextData.some(
            (updatedItem: any) => updatedItem.ref === item.ref,
          ),
      );
